...
Circles, Voronoi Hull) show presence (green) vs absence (grey). For GeoDist, Convex Hull, GeoIDW and
Voronoi Hull the two columns are visually indistinguishable (Pearson r = 1.000). The Circles
EcoCommons panel is blank because the platform's projection step returned an all-`NA` raster under
the local `dismo 1.3.16` / `terra 1.9.11` — a package-version artifact, not a model difference (see
below).*
